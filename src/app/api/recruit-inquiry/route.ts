...
export async function POST(request: NextRequest) {
  ...
  const upsertRes = await fetch(`${GHL_BASE}/contacts/upsert`, {
    ...
    body: JSON.stringify({
      ...
      tags,
      source: `Landing Page - teamlendwise.com (ref=${slug})`,
    }),
  });
  ...
  await fetch(`${GHL_BASE}/contacts/${contactId}/notes`, {
    ...
    body: JSON.stringify({
      body:
        `Inbound recruit submission — teamlendwise.com\n\n` +
...
function buildNotifyEmailHtml(p: {
  ...
  <h2 style="color:#0a1628;border-bottom:2px solid #C9A227;padding-bottom:8px;">
    New Recruit Lead — teamlendwise.com
  </h2>
